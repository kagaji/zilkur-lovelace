...
async function renderAndPostAsync(browser) {
    for (let pageIndex = 0; pageIndex < config.pages.length; pageIndex++) {
        ...
        const html = await renderUrlToHtmlAsync(browser, pageConfig, url);
        ...
        const options = {
            ...
            headers: {
                ...
                "Content-Length": Buffer.byteLength(htmlData)
            },
            rejectUnauthorized: !config.ignoreCertificateErrors
        };
        const zilkurUrl = `http://192.168.1.99:3000/api/plugins/homeassistant`;
        const httpLib = zilkurUrl.toLowerCase().startsWith("https") ? https : http;
        const req = httpLib.request(url, options, (res) => {
            if (res.statusCode !== 200) {
                console.error(
                    `Update device status ${res.statusCode}: ${res.statusMessage}`
                );
            }
        });
        req.on("error", (e) => {
...
async function renderUrlToHtmlAsync(browser, pageConfig, url) {
    let page;
    let html;
    try {
